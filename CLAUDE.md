...
│   ├── models/                ← ORM models: prompt, script, script_template, tag (+ base)
│   ├── schemas/               ← Pydantic validation per entity
│   ├── services/              ← business logic / all DB access (incl. generation_service)
│   ├── routes/                ← prompts, generate, scripts, script_templates, tags, pages, settings, health, pwa, common
│   ├── templates/
...
- **Database:** `init_db()` runs `create_all` on startup for dev/test convenience. For production schema changes, adopt Alembic and make `init_db()` a no-op (see [CUSTOMIZATION.md](CUSTOMIZATION.md)). Tests point `DATABASE_URL` at a throwaway SQLite file and rebuild the schema per test ([tests/conftest.py](tests/conftest.py)).
